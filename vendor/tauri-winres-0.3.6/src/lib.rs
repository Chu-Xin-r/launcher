...
fn handle_compilation_result(result: embed_resource::CompilationResult) -> Result<(), embed_resource::CompilationResult> {
    match result {
        embed_resource::CompilationResult::NotAttempted(reason)
            if env::var_os("PROFILE").as_deref() == Some(std::ffi::OsStr::new("debug")) => {
                println!("cargo:warning=Windows resource compiler unavailable; debug build continues without embedded icon/manifest: {reason}");
                Ok(())
            }
        other => other.manifest_required(),
    }
}

fn configure_gnu_tools_path() {
    #[cfg(all(windows, target_env = "gnu"))]
    if Path::new(r"E:\MinGw\bin\windres.exe").is_file() {
        let mut path = OsString::from(r"E:\MinGw\bin");
...
impl WindowsResource {
    ...
    pub fn set_icon_with_id<'a>(&mut self, path: &'a str, name_id: &'a str) -> &mut Self {
        let icon_path = {
            #[cfg(all(windows, target_env = "gnu"))]
            {
                // GNU windres cannot reliably open resource paths containing non-ASCII
                // characters. Stage the icon beside the ASCII build output first.
                let source = Path::new(path);
                let staged = env::var_os("OUT_DIR").map(|out_dir| {
                    let filename = format!("launcher-icon-{}.ico", name_id);
    ...
    pub fn compile(&self) -> io::Result<()> {
        ...
        // This matches v2 behavior. A debug build may proceed without cosmetic
        // Windows resources when no resource compiler is installed; release
        // builds remain strict so they cannot silently ship without metadata.
        configure_gnu_tools_path();
        handle_compilation_result(embed_resource::compile(rc, embed_resource::NONE))
    ...
    pub fn compile_for(&self, binaries: &[&str]) -> io::Result<()> {
        ...
        // Windows resources when no resource compiler is installed; release
        // builds remain strict so they cannot silently ship without metadata.
        handle_compilation_result(embed_resource::compile_for(rc, binaries, embed_resource::NONE))
            .map_err(|error| io::Error::new(io::ErrorKind::Other, error))?;
